...
def convert_to_sftrainer_format(file_prefix, input_dir, output_dir):
    ...
    template_names = [f for f in os.listdir(input_dir) if f!= ".DS_Store"]
    
    for template_name in template_names:
        file_path = os.path.join(input_dir, template_name, f"{file_prefix}_random_sos_aug_{template_name}.json")
    ...
    for i in tqdm(range(n_samples), desc=f"Processing {os.path.basename(file_path)}"):
        ...
        base_entry["messages_optimal"] = [
                {"role": "user", "content": user_prompt.format(target=base_entry["target"],nums=base_entry["nums"],)}, 
    ...
    output_json_path_search = os.path.join(output_dir, f"{file_prefix.replace("val","test")}.json")
...
def convert_to_hf(input_dir, output_dir):
    ...
    file_prefixes = [f.split("_random_sos_aug_")[0] for f in file_names]
